...
public class SaveOrder implements WebService {
  ...
  /**
   * Queue payload normalized to internal native shape expected by OCOrder pipeline.
   */
  private JSONObject normalizeForQueue(JSONObject requestBody) throws JSONException {
    ...
    normalized.put("posTerminal", requestBody.optString("posTerminal", null));
    normalized.put("appName", "OCRE");
    normalized.put("channel", "Native");

    JSONArray data = new JSONArray();
    data.put(new JSONObject(requestBody.getJSONObject("order").toString()));
