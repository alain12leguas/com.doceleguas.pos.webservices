 ...
 * 
 * This class constructs and executes native SQL queries to retrieve a single order from the C_Order
 * table, following the same pattern as PaidReceipts.
 * 
...
public class OrderModel extends Model {
  ...
  //@formatter:off
    String sqlLines = "SELECT " 
        ...
        + "ol.em_obpos_isdeleted as \"isDeleted\", " + "ol.c_return_reason_id as \"returnReason\", "
        + "rr.name as \"returnReasonName\", " + "ol.c_order_discount_id as \"orderDiscount\", "
        ...
        + "ol.qtydelivered as \"deliveryQuantity\", " + "(ol.qtydelivered - ( "
        + "	    SELECT COALESCE(ABS(SUM(ol1.qtydelivered)), 0) "
        + "	    FROM c_orderline ol1 "
        + "     JOIN m_inoutline gsl ON gsl.m_inoutline_id = ol1.m_inoutline_id"
        + "	    WHERE gsl.c_orderline_id = ol.c_orderline_id "
        + "	)) AS \"remainingQuantity\" " + "FROM c_orderline ol "
        ...
        + "WHERE ol.c_order_id = :orderId " + "AND COALESCE(ol.em_obpos_isdeleted, 'N') = 'N' "
        + "ORDER BY ol.line";
  //@formatter:on
  ...
  /**
   * Retrieves taxes for a specific order line. Equivalent to PaidReceipts taxes per line.
   * 
   * @param lineId
   *          The order line UUID
  ...
  private JSONArray getLineTaxes(String lineId) throws JSONException {
    ...
    String sqlTaxes = "SELECT " 
        ...
        + "WHERE olt.c_orderline_id = :lineId " + "ORDER BY olt.line";
    //@formatter:on
  ...
  /**
   * Retrieves promotions (discounts) for a specific order line. Equivalent to PaidReceipts
   * promotions per line.
   * 
   * @param lineId
   *          The order line UUID
  ...
  private JSONArray getLinePromotions(String lineId) throws JSONException {
    ...
    String sqlPromotions = "SELECT " 
        ...
        + "olo.em_obdisc_identifier as \"discIdentifier\", " + "olo.line as \"lineNo\" "
        + "FROM c_orderline_offer olo " + "JOIN m_offer o ON olo.m_offer_id = o.m_offer_id "
        + "WHERE olo.c_orderline_id = :lineId " + "ORDER BY olo.line";
    //@formatter:on
    NativeQuery<?> queryPromotions = OBDal.getInstance()
        .getSession()
        .createNativeQuery(sqlPromotions);
  ...
  /**
   * Retrieves payments (receiptPayments) for the given order. Equivalent to
   * PaidReceipts.receiptPayments.
   * 
   * @param orderId
   *          The order UUID
  ...
  public JSONArray getReceiptPayments(String orderId) throws JSONException {
    ...
    String sqlPayments = "SELECT "
        ...
        + "pm.fin_paymentmethod_id, c.c_currency_id, c.iso_code, fp.em_obpos_app_cashup_id, "
  ...
  /**
   * Retrieves tax summary (receiptTaxes) for the given order. Equivalent to
   * PaidReceipts.receiptTaxes.
   * 
   * @param orderId
   *          The order UUID
  ...
  public JSONArray getReceiptTaxes(String orderId) throws JSONException {
    ...
    String sqlTaxes = "SELECT " 
        ...
        // + "t.isspecialtax as \"isSpecialTax\" "
        + "FROM c_ordertax ot " + "JOIN c_tax t ON ot.c_tax_id = t.c_tax_id "
        + "WHERE ot.c_order_id = :orderId " + "ORDER BY ot.line";
    //@formatter:on
    NativeQuery<?> queryTaxes = OBDal.getInstance().getSession().createNativeQuery(sqlTaxes);
