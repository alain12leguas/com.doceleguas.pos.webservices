 ...
 * 
 * <p>
 * This class centralizes common functionality used by both {@link OrderModel} and
 * {@link OrdersFilterModel} to avoid code duplication.
 * </p>
 * 
...
public final class OrderQueryHelper {
  ...
  /**
   * SQL subquery to calculate deliveryMode from order lines. Returns the maximum delivery mode,
   * defaulting to 'PickAndCarry'. Equivalent to PaidReceiptsFilterProperties.deliveryMode
   */
  public static final String DELIVERY_MODE_SQL = "(SELECT COALESCE(MAX(ol.em_obrdm_delivery_mode), 'PickAndCarry') "
      + "FROM c_orderline ol " + "WHERE ol.c_order_id = ord.c_order_id "
      + "AND COALESCE(ol.em_obpos_isdeleted, 'N') = 'N')";

  /**
   * SQL subquery to calculate deliveryDate from order lines. Returns the minimum combined delivery
   * date/time using to_timestamp for PostgreSQL compatibility. Equivalent to
   * PaidReceiptsFilterProperties.deliveryDate
   */
  // @formatter: off
  public static final String DELIVERY_DATE_SQL = //
      "(SELECT MIN(CASE "
          ...
          + "ELSE to_timestamp(" + "to_char(ol.em_obrdm_delivery_date, 'YYYY') || '-' || "
          + "to_char(ol.em_obrdm_delivery_date, 'MM') || '-' || "
          + "to_char(ol.em_obrdm_delivery_date, 'DD') || ' ' || "
          + "to_char(ol.em_obrdm_delivery_time, 'HH24') || ':' || "
          + "to_char(ol.em_obrdm_delivery_time, 'MI'), 'YYYY-MM-DD HH24:MI') " + "END) "
          + "FROM c_orderline ol " + "WHERE ol.c_order_id = ord.c_order_id)";
  // @formatter: on
  /**
   * SQL CASE expression to determine orderType. Equivalent to
   * PaidReceiptsFilterProperties.getOrderType() default case.
   * 
   * <p>
   * Order types:
   * </p>
  ...
  public static final String ORDER_TYPE_SQL = //
      "(CASE " + "WHEN doctype.isreturn = 'Y' THEN 'RET' "
          + "WHEN doctype.docsubtypeso = 'OB' THEN 'QT' "
          + "WHEN COALESCE(ord.em_obpos_islayaway, 'N') = 'Y' THEN 'LAY' " + "ELSE 'ORD' END)";
  // @formatter: on
  /**
   * SQL subquery to calculate paidAmount from payment schedules. Returns the sum of paid amounts
   * for the order. HQL equivalent: (select coalesce(sum(fps.paidAmount), 0) from
   * FIN_Payment_Schedule fps where fps.order = ord)
   */
  public static final String PAID_AMOUNT_SQL = "(SELECT COALESCE(SUM(fps.paidamt), 0) "
      + "FROM fin_payment_schedule fps " + "WHERE fps.c_order_id = ord.c_order_id)";

  /**
   * SQL CASE expression to determine order status. Calculates status based on document type,
  ...
  public static final String STATUS_SQL = //
      "(CASE " + "WHEN doctype.isreturn = 'Y' THEN 'Refunded' "
          + "WHEN doctype.docsubtypeso = 'OB' THEN 'Under Evaluation' "
          + "WHEN ord.iscancelled = 'Y' THEN 'Cancelled' " + "WHEN ord.grandtotal <= 0 THEN 'Paid' "
          + "ELSE (SELECT CASE " + "WHEN COALESCE(SUM(fps.paidamt), 0) = 0 THEN 'UnPaid' "
          + "WHEN COALESCE(SUM(fps.paidamt), 0) < ord.grandtotal THEN 'Partially Paid' "
          + "ELSE 'Paid' END "
          + "FROM fin_payment_schedule fps WHERE fps.c_order_id = ord.c_order_id) " + "END)";
  // @formatter: on
  /**
   * SQL EXISTS expression to check if invoice was created. Checks if any invoice line exists linked
   * to this order's lines with simplified invoice sequence.
   */
  public static final String INVOICE_CREATED_SQL = "(EXISTS(SELECT 1 FROM c_invoiceline il "
  ...
  /**
   * SQL expression to check if order has verified returns. Checks if any order line has a goods
   * shipment line linked to a return order line.
   */
  // @formatter: off
  ...
  // @formatter: on
  /**
   * SQL CASE expression to check if order has negative quantity lines. Returns true if any order
   * line has orderedQuantity < 0.
   */
  public static final String HAS_NEGATIVE_LINES_SQL = "(CASE WHEN EXISTS (SELECT 1 FROM c_orderline ordLine "
      + "WHERE ordLine.c_order_id = ord.c_order_id "
      + "AND ordLine.qtyordered < 0) THEN true ELSE false END)";

  /**
   * SQL expression to check if order is a quotation (has quotation reference). Returns true if
   * ord.quotation_id is not null.
   */
  public static final String IS_QUOTATION_SQL = "(CASE WHEN ord.quotation_id IS NOT NULL THEN true ELSE false END)";

  /**
   * Common JOIN clauses for order queries. Joins c_order with related tables commonly needed in
   * order queries.
   */
  // @formatter: off
  public static final String ORDER_BASE_JOINS = //
      " INNER JOIN c_currency currency ON currency.c_currency_id= ord.c_currency_id"
  ...
  private static final String JOIN_OBPOS = " LEFT JOIN obpos_applications obpos ON ord.em_obpos_applications_id = obpos.obpos_applications_id";
  private static final String JOIN_ORG = " LEFT JOIN ad_org org ON ord.ad_org_id = org.ad_org_id";
  private static final String JOIN_TRXORG = " LEFT JOIN ad_org trxorg ON obpos.ad_org_id = trxorg.ad_org_id";
  private static final String JOIN_BP = " LEFT JOIN c_bpartner bp ON ord.c_bpartner_id = bp.c_bpartner_id";
  private static final String JOIN_SALESREP = " LEFT JOIN ad_user salesrep ON ord.salesrep_id = salesrep.ad_user_id";
  private static final String JOIN_DOCTYPE = " LEFT JOIN c_doctype doctype ON ord.c_doctypetarget_id = doctype.c_doctype_id";
  ...
  /**
   * Builds a JOIN clause including only the JOINs whose table aliases are referenced by the
   * provided SQL fragments (SELECT list, WHERE clause, ORDER BY, etc.).
   * 
   * <p>
   * This avoids joining tables that are never referenced in the query, significantly reducing I/O —
   * especially for COUNT queries where only the WHERE clause matters.
   * </p>
   * 
   * @param sqlParts
   *          One or more SQL text fragments to analyze for alias references
  ...
  public static String buildConditionalJoins(String... sqlParts) {
    ...
    if (needsObpos) {
      joins.append(JOIN_OBPOS);
    }
    if (needsOrg) {
      joins.append(JOIN_ORG);
    }
    if (needsTrxorg) {
      joins.append(JOIN_TRXORG);
    }
    if (needsBp) {
      joins.append(JOIN_BP);
    }
    if (needsSalesrep) {
      joins.append(JOIN_SALESREP);
    }
    if (needsDoctype) {
      joins.append(JOIN_DOCTYPE);
    }
  ...
  /**
   * Checks if the SQL text references a specific table alias (e.g. "bp.", "org."). Uses
   * word-boundary matching to avoid false positives — for example, "trxorg.name" will NOT match
   * alias "org".
   */
  ...
  /**
   * Private constructor to prevent instantiation. This is a utility class with only static members.
   */
  ...
  /**
   * Sanitizes the SELECT list to prevent SQL injection. Removes dangerous SQL keywords while
   * preserving valid column expressions.
   * 
   * @param selectList
   *          Raw SELECT list from request
   ...
   * 
   * @param orderBy
   *          Raw ORDER BY clause from request
   ...
   * 
   * <p>
   * This method allows clients to request computed properties using simple aliases like
   * "@deliveryMode", "@deliveryDate", "@orderType" instead of complex SQL subqueries. The aliases
   * are replaced with their corresponding SQL expressions before query execution.
   * </p>
   * 
   * <p>
   ...
   * </p>
   * <ul>
   * <li>{@code @deliveryMode} - Maximum delivery mode from order lines (default:
   * 'PickAndCarry')</li>
   ...
   * <li>{@code @status} - Order status: Refunded, UnderEvaluation, Cancelled, UnPaid,
   * PartiallyPaid, Paid</li>
   ...
   * 
   * @param selectList
   *          The SELECT list potentially containing computed property aliases
  ...
  /**
   * Converts a result row Map to a JSON object. Handles null values by converting them to
   * JSONObject.NULL.
   * 
   * @param rowMap
   *          Map of column names to values from the query result
