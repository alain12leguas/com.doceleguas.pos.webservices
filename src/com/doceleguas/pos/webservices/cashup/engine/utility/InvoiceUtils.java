...
package com.doceleguas.pos.webservices.cashup.engine.utility;
...
import com.doceleguas.pos.webservices.cashup.engine.CashupPOSUtils;
import com.doceleguas.pos.webservices.cashup.engine.TicketPropertyMapping;
...
public class InvoiceUtils implements TicketPropertyMapping {
  ...
  private void createInvoiceAndLines(final JSONObject jsoninvoice, final Invoice invoice,
    ...
    final OBCriteria<Invoice> obCriteria = OBDal.getInstance()
    .createCriteria(Invoice.class);
    obCriteria.setFilterOnActive(true);
    obCriteria.add(Restrictions.eq(Invoice.PROPERTY_DOCUMENTNO, invoice.getDocumentNo()));
    
    invoice.setDocumentNo(order.getDocumentNo());
    if (invoice.isNewOBObject() && !obCriteria.list().isEmpty()) {
    	invoice.setDocumentNo(invoice.getDocumentNo() + "/A");
    	System.out.println("LOG::INFO::InvoiceUtils::createInvoiceAndLines->Invoice DocumentNo repeated: " + order.getDocumentNo());
    }
    
    OBDal.getInstance().save(invoice);
    createInvoiceLines(invoice, order, jsoninvoice, invoicelines, invoicelineReferences);

    if (CashupPOSUtils.isCrossStore(order, order.getObposApplications())) {
  ...
  private void createInvoiceLine(Invoice invoice, Order order, JSONObject jsoninvoice,
    ...
    invoiceLine.setWpscusPriceactual(
        getUnitPriceUnrounded(jsonInvoiceLine).setScale(6, RoundingMode.HALF_UP));
  ...
  public FIN_PaymentSchedule createPSInvoice(Order order, Invoice invoice) {
    ...
    for (int i = 0; i < paymentScheduleList.size(); i++) {
      ...
      boolean invoicePaidAmounts = CashupPOSUtils.isPaidStatus(psd.getPaymentDetails().getFinPayment());
