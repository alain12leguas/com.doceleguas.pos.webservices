...
- `SaveOrder` / `OCWS_Order` already run through native pipeline:
  ...
  - No runtime delegation to `ExternalOrderLoader` / `OrderLoader` in OCWS_Order flow.
- Web POS dependency is still required by **other** webservices in this module.

## Checklist before deleting the dependency row

1. Keep `SaveOrder`/`OCWS_Order` regression green in `native` mode with golden payloads and SQL parity checks (`doc/regression-suite-OCWS_Order.md`).
