...
## Motor nativo equivalente a OrderLoader (OCWS_Order)
...
Esta clase implementa el rol de persistencia que antes recaía en runtime sobre `org.openbravo.retail.posterminal.OrderLoader` para el flujo `OCWS_Order`:
...
1. `SaveOrder.doPost` recibe contrato nativo (`messageId`, `posTerminal`, `order`) y crea `C_IMPORT_ENTRY` tipo `OCWS_Order`.
2. `OCOrderImportEntryProcessor` despacha la fila a `OCOrderImportRunnable` (instancia directa, sin proxy CDI de cliente).
